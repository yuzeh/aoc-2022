    ...
    def draw_rock(self, rock_path):
        ...
        for i in range(len(rock_path) - 1):
            for point in rock_path[i].iter_points_to(rock_path[i + 1]):
                ...
                if self.max_rock_y is None or self.max_rock_y < point.y:
                    self.max_rock_y = point.y

    def drip_sand(self):
        """Simulates one unit of sand dropping through from 500,0 onto the grid.

        Returns True if there is any more place for sand to fall, False if not.
        """
        if self.point_at(500, 0):
            return False

        sx, sy = 500, 0
        while True:
            # if we've escaped the bounds of the map then we're gonna fall forever
            if sy > self.max_rock_y:
                self.coords[sx, sy] = 'o'
                return True
